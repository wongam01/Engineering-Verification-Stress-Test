...
RAW_TEXT_FILE = (
    OUTPUT_DIR
    / "blind_source_text_run_01.txt"
)

PREPROCESS_FILE = (
    OUTPUT_DIR
    / "blind_preprocess_run_01.json"
)
...
def main() -> None:
    ...
    if RAW_TEXT_FILE.exists():
        raise FileExistsError(
            ...
            "Blind Run #1 source text will not be overwritten."
        )

    if PREPROCESS_FILE.exists():
        raise FileExistsError(
            f"{PREPROCESS_FILE.name} already exists.\n"
            "Blind Run #1 preprocessing result will not be overwritten."
        )
    ...
    artifact = {
        "metadata": {
            "stage": (
                "PROTOTYPE_BLIND_PREPROCESS_RUN_01"
            ),
    ...
    print(
        "=== PROTOTYPE BLIND PREPROCESS RUN #1 ==="
    )
    print(
