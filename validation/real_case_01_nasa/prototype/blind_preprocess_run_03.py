...
def main() -> None:
    ...
    if RAW_TEXT_FILE.exists():
        raise FileExistsError(
            ...
            "Blind Run #3 source text will not be overwritten."
        )

    if PREPROCESS_FILE.exists():
        raise FileExistsError(
            f"{PREPROCESS_FILE.name} already exists.\n"
            "Blind Run #3 preprocessing result will not be overwritten."
        )
    ...
    artifact = {
        "metadata": {
            "stage": (
                "PROTOTYPE_BLIND_PREPROCESS_RUN_03"
            ),
    ...
    print(
        "=== PROTOTYPE BLIND PREPROCESS RUN #3 ==="
    )
    print(
